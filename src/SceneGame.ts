...
class SceneGame extends eui.Component{
    ...
    private retrybtn:eui.Image;
    public list_sel:eui.List;
    public list_confuse:eui.List;
    private winUI:eui.Group;
    private sonename:eui.Label;
    private tip:eui.Label;
    private nextItembtn:eui.Button;
    private mainUIbtn:eui.Button;
    //其他变量
    private sourceCollection:eui.ArrayCollection;
    private selCollection:eui.ArrayCollection;
    private isFull:boolean;          //list_sel是否填满
    private aMyanswer:any[] = [];    //存储己方答案
    private aRightanswer:any[] = [];   //存储正确答案 待 初始化
    private selectedIdx:number;
    private relativelain={         //sel和confuse对应关系
        sel:[],
        confuse:[]
    };    
    ...
    private selWord(){
        ...
        for(let i=0;i < this.aRightanswer.length;i++){
    ...
    private selChange(){
        ...
        let sTemp = this.selCollection.getItemAt(this.selectedIdx).label;
        let oTemp:Object;
        //console.log('this.relativelain.confuse[this.selectedIdx]:'+this.relativelain.confuse[this.selectedIdx])
        let posTemp = this.relativelain.confuse[this.selectedIdx];
    ...
    private itemChange(){
        ...
        //判断第一个为' '
        let temp = BaseUtil.eleIdx(this.aMyanswer,-1);
        if(temp == -1){
            isContinue = false;
        }else{
            isContinue = true;
        ...
        if(isContinue){
            ...
            let sTemp = this.sourceCollection.getItemAt(this.selectedIdx).label;
    ...
    private collectionChangeHandler(){
    ...
    private checkAnswer(){        
        ...
        if(this.isFull==true){
            let smya = this.aMyanswer.join("");
            let srighta = this.aRightanswer.join('');
            if(smya == srighta){
    ...
    //存储正确答案
    private storageRight(){
        let sTemp = BaseUtil.removeSpace(this.levelData["song_name"]);
        let eTemp:any;
        this.aRightanswer = [];
    ...
    private backLevelscene(){
        ...
        //网格布局
    public gridlayout(obj:any,colNum:number){
...
//创建ItemRenderer
class WordlabelRender extends eui.ItemRenderer{
    private wordLabel:eui.Label;
    private temp:string;
    private tempwordidx:number;
    private temp_word:eui.List = SceneGame.Shared().list_confuse;
...
class BaseUtil{
    ...
    //push进数组的元素是唯一的
    public static pushonly(arr:any[],ele:any){
        if(arr.indexOf(ele) == -1){
            arr.push(ele);
        }
    }
    //去除字符串中个空格
    public static removeSpace(s:string){
        let temp = s.replace(/[]/g,'');
        return temp;
    }
    //去除数组中''所在的元素
    public static removeArrayspace(a){
        let idx:number;
        let cur:any;
